...
@foreach($data as $ev)
  <item>
	...
	<description>{{ trim(preg_replace(['~<[^>]+>~si', '~[ \s]+~si'], ['', ' '], html_entity_decode(str_replace('', '', $ev['AdditionalDescription1'])))) }}</description>
	...
	<title>{{ $ev['TypeOfNoticeDescription'] }}: {{ html_entity_decode(str_replace('', '', $ev['ShortTitle'])) }}</title>
  </item>
@endforeach
</channel>
</rss>